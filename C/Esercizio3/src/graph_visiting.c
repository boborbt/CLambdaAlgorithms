...
VisitingInfo VisitingInfo_new(Graph graph, void (*visit)(const void*, void*), void* user_info) {
...
void depth_first_visit(VisitingInfo info, const void* source) {
  ...
  while(!EdgeIterator_end(adj_it)) {
    const void* neighbour = EdgeIterator_get(adj_it).vertex;
    const void* dummy;
